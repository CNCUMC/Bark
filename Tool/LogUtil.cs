...
public static class LogUtil
{
    ...
    public static void Info(string text, ManualLogSource? logger) { LogToConsole(text); logger?.LogInfo(text); }
    public static void Error(string text, ManualLogSource? logger) { LogToConsole($"[ERROR] {text}"); logger?.LogError(text); }
    public static void Warning(string text, ManualLogSource? logger) { LogToConsole($"[WARNING] {text}"); logger?.LogWarning(text); }

    public static void CheckWorld(ManualLogSource logger)
    {
        if (!GameInstances.IsInGame)
            throw new InvalidOperationException(Locale("world.check_for_world"));
    }
    ...
    public static void CheckArgumentCount(string[] args, int minCount, ManualLogSource logger)
    {
        ...
        if (args.Length <= minCount)
            throw new Exception(Locale("utils.check_argument_count", minCount, args.Length - 1));
    }
    ...
    {
        Divider(); Info(header, logger);
        foreach (var (gn, items) in groups) { Info($"{groupPrefix}{gn}:", logger); foreach (var it in items) Info($"{itemPrefix}{it}", logger); }
        Divider();
    }

    public static void PrintEmpty(string message, ManualLogSource logger) => Info(message, logger);
