...
`"type": "gun"` — a fireable gun item. The template presets the `wolfsrife` prefab (the base game's rifle logic), then overrides its behavior via Harmony patches.
...
| `capacity`             | int    | `15`                   | Tube capacity for direct-loading guns (shotguns etc.). Overridden by mag capacity when a mag is present |
