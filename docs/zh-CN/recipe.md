[English](../en-US/recipe.md) | ***简体中文***

# 自定义合成表

在模组目录下创建 `Recipe/` 文件夹，放入 JSON 文件即可注册自定义合成表。Bark 会在模组加载时自动扫描并注册到 CUCoreLib 的 `RecipeRegistry`。
...
| `int` | int | 0 | 制作所需智力 |
| `category` | string | `"Materials"` | 蓝图分类：`Materials`, `Tools`, `Medicine`, `Utilities`, `Food` |
...
`RecipeEntry` 字段：
- `Id`：配方产物 ID
- `FileName`：来源 JSON 文件名（如 `"bandage.json"`）

## 热重载

修改 `Recipe/` 下的 JSON 文件后，在控制台执行 `reload` 即可重载所有模组配方。`RecipeLoader` 会先清除该模组的旧配方再注册新的，避免新旧配方共存。

## 与物品系统联动

自定义合成表的 `id` 可以是原版物品，也可以是 [`Assets/Item/` 目录下定义的自定义物品](items.md)。两者配合可构建完整的自定义物品生态系统。
