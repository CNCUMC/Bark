...
通过 JSON 定义自定义角色状态（Moodle），如流血、感染、中毒等。JSON 文件放在脚本模组的 `Moodle/` 目录下，精灵图片放在
`Assets/Moodle/` 下。

Moodle 系统基于游戏内建的状态队列机制——每次应用 Moodle 都会根据 `hold_seconds` 自动设定到期时间，到期后自动消失并触发
`onMoodleLose` 事件。
...
如果以上全部失败，该 Moodle 将 **跳过注册**并输出 warning 日志。这意味着你甚至可以不写任何图标字段——只要在
`Assets/Moodle/` 下放一张与 key 同名的 `.png` 图片即可。
...
> ℹ️ 两个模组定义相同 key 的 Moodle 时，后加载的覆盖先加载的。key 用于全局唯一引用，脚本中调用 `Moodle.ApplyMoodle()`
> 时传入的就是这个 key。
