...
| `gun_type`              | string | `"rifle"`              | 枪型类别，决定默认音效/精灵/枪口粒子按哪种枪械分支：`"pistol"` / `"rifle"` / `"shotgun"`。由作者显式指定，**不要按弹药口径推断**（口径 ≠ 枪型） |
| `damage`                | float  | `42`                   | 单发伤害                                                                                                                                       |
| `fire_sound`            | string | `""`                   | 开火音效路径，相对于模组根目录，如 `"Assets/Audio/ak47_shot.wav"`。纯文件名自动补全 `Assets/Audio/` 前缀。为空则按 `gun_type` 自动选默认音效  |
