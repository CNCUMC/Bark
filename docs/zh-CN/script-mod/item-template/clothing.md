...
`"type": "clothing"` — 可穿戴的服装物品。模板预设 `geofruit` 预制体及合理的默认值（utility 分类、不腐烂、0 护甲/隔离等），通过原版
`wearable` 系统实现装备与卸下。

> 📘 衣服模板负责注册和预设默认值。容器（`container`）和电池（`battery`）等扩展功能使用原版 `ItemDef`
> 对应字段。非原版的运行时行为（头灯、攀爬、潜水等）由脚本系统实现。
