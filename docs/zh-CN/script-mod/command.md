...
通过 JSON 在 `Command/` 目录下注册控制台命令，输入命令后 Bark 通过 `onCommand` 事件分发到脚本侧。命令使用游戏自带的
`ConsoleCommandRegistry` 注册，支持参数描述和自动完成。
